...
class LoopPatcher:
    ...
    def create(
        ...
        return patcher._write_patch(
            output_path, base_crc, blocks, block_meta, crc_data, len(records), metadata or {}
        )
    ...
    def _load_records(self, source: Union[str, Path, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    ...
    def _compress_block(self, records: List[Dict[str, Any]], block_idx: int) -> tuple:
    ...
    def _write_patch(
        ...
        with open(output_path, "wb") as f:
            ...
            # Footer (8 bytes)
            footer = struct.pack("<I", len(meta_bytes)) + MAGIC_PATCH_FOOTER
    ...
    def _read_patch(self, patch_path: Path) -> Dict[str, Any]:
        ...
        with open(patch_path, "rb") as f:
            ...
            for _ in range(n_blocks):
                ...
                offset, comp_size, uncomp_size, n_rec, split_id, _ = struct.unpack("<QIIIHH", raw)
                index.append(
                    {
                        "offset": offset,
    ...
    def _merge(self, base_reader: LoopReader, patch_data: Dict[str, Any], output_path: Path) -> int:
